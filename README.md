...
Use **Save** at the top right to download a versioned `.tvn-project` file, and **Load** to reopen one. It embeds the original JPEGs together with the route, map choice, timing, crop model, overlay options, manual photo edits, mixed letters, and accepted exceptions. The schema version is stored inside the file and included in its filename; version-specific readers allow older project schemas to remain supported as the format evolves. Loading restores state locally without repeating OSM discovery or DOF025 downloads; OSM tile consent is intentionally requested again. Project files can be large because their photos are embedded and should be handled as private event material.

After generating the normal package, **Prepare speed editions** creates a ZIP containing `judge_solution_map_all_speeds.pdf` and `competitor_route_map_all_speeds.pdf`. Each has one page per default speed (50–100 kt in 5 kt steps), followed by configured non-duplicate custom speeds. Compact red speed text (for example, `70kt`) sits beside each route. A4/A3 selection includes a 12 mm protective margin without separate header/footer bands. PDFs request an initial view focused on the route, Actual size printing, and paper selection by page size. Screen zoom does not change physical map scale. The `shared/` folder retains the empty map, rulers, photo handouts, CSVs, and restorable project. PDF viewers and printer drivers may override preferences; verify the 100 mm check line on paper.
...
**Find SP/TP/FP photo options** searches for the nearest identifiable OSM object within 3 km of every control to determine the matching object type. Every true SP, TP, and FP DOF025 crop is centered on the exact supplied waypoint coordinates; SP and FP are fixed to that true crop. For each TP, the app also uses the matched OSM type to search bounded sample areas up to 10 NM from the exact waypoint and accepts a false option only when it is at least 1 NM away. The organizer reviews DOF025 previews and chooses true or false for every TP before import. Control photos retain their linked waypoint and solution role, are kept separate from the 12 en-route letter photos, and appear as a separate section in judge and competitor handouts; the competitor handout does not reveal whether a TP photo is true or false.
