...
After generating the normal package, **Prepare speed editions** creates a ZIP containing `judge_solution_map_all_speeds.pdf` and `competitor_route_map_all_speeds.pdf`. Each has one page per default speed (50–100 kt in 5 kt steps), followed by configured non-duplicate custom speeds. Every map page carries a large groundspeed heading. Page size is A4 or A3, including reserved header/footer space, with PDF preferences requesting Actual size and paper selection by page size. The `shared/` folder retains the empty map, rulers, photo handouts, CSVs, and restorable project. PDF viewers and printer drivers may override preferences; verify the 100 mm check line on paper.
