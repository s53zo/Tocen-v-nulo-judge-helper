...
After generating the normal package, **Prepare speed editions** creates a ZIP with separate judge solution and competitor route maps for the default 50, 55, 60, …, 100 kt set and any configured custom speeds. Route setup provides four reusable custom slots, each supporting knots or km/h. Duplicate-equivalent custom speeds are omitted. A `shared/` folder contains the empty map, judge and competitor photo handouts, photo CSV files when photos exist, and a versioned `.tvn-project` file with the embedded source photos. Speed-dependent route summaries are intentionally not copied as shared files.
